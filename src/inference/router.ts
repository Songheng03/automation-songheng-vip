...
export class InferenceRouter {
  ...
  async route(
    ...
    // 8b. Deduct real cost from virtual credits
    try {
      const row = this.db.prepare("SELECT value FROM kv WHERE key = 'virtual_credit_balance'").get() as any;
      if (row) {
        const current = parseInt(row.value, 10);
        const newBalance = Math.max(0, current - actualCostCents);
        this.db.prepare("UPDATE kv SET value = ? WHERE key = 'virtual_credit_balance'").run(String(newBalance));
        if (actualCostCents > 0) {
          console.log(`[credits] -${actualCostCents} inference: ${model.modelId} (${current} -> ${newBalance})`);
        }
      }
    } catch (e) {
      console.error("[credits] deduct error:", e instanceof Error ? e.message : e);
    }
