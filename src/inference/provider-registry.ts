...
const DEFAULT_PROVIDERS: ProviderConfig[] = [
  {
    ...
    models: [
      ...
      // Qwen models (via overrideBaseUrl in loop.ts)
      {
        id: "qwen3.7-max",
        tier: "reasoning",
        contextWindow: 128000,
...
export class ProviderRegistry {
  ...
  /** Override model IDs for a provider (e.g. when switching to a compatible API) */
  overrideModelIds(providerId: string, modelMap: Record<string, string>): void {
    const provider = this.providers.find((p) => p.id === providerId);
    if (!provider) return;
    for (const model of provider.models) {
      if (modelMap[model.id]) {
        model.id = modelMap[model.id];
      }
    }
  }
  ...
  private isProviderActive(provider: ProviderConfig): boolean {
    ...
    // Skip providers whose API key isn't configured. The local provider
    // (Ollama/local LLM) is exempt — a missing key there is fine.
    if (provider.id !== "local") {
      const configured = process.env[provider.apiKeyEnvVar];
      if (typeof configured !== "string" || configured.length === 0) {
        return false;
