...
function sanitizeContent(content: string): string {
  // Fix incomplete hex/unicode escape sequences that cause JSON parse errors
  // when content is serialized. These arise from corrupted turn data or tool results.
  // Remove \x followed by <2 hex chars, or \u followed by <4 hex chars
  return content.replace(/\\(?:x(?![0-9a-fA-F]{2})|u(?![0-9a-fA-F]{4}))/g, "");
}
