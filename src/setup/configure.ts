...
function printMainMenu(config: AutomatonConfig): void {
  const providers = [
    config.openaiApiKey ? (config.openaiBaseUrl ? `OpenAI-compat(${new URL(config.openaiBaseUrl).hostname})` : "OpenAI") : null,
...
async function configureProviders(config: AutomatonConfig): Promise<void> {
  ...
  config.openaiApiKey = await askString("API key (leave empty for local mode)", config.openaiApiKey) || undefined;

  config.openaiApiKey = await askString("OpenAI API key  (sk-...)", config.openaiApiKey) || undefined;
  config.openaiBaseUrl = await askString("OpenAI-compatible base URL  (https://api.deepseek.com)", config.openaiBaseUrl) || undefined;
