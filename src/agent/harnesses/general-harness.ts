...
/** Detect self-kill attempts (agent trying to kill its own process) */
function isSelfKill(command: string): boolean {
  const selfPid = process.pid;
  // Match kill <PID> or kill -SIGTERM/-9/-15 <PID> or kill -s TERM <PID>
  const killMatch = command.match(
    /\bkill\s+(?:-(?:\d+|SIG\w+)\s+)?(?:-s\s+\w+\s+)?(\d+)/,
  );
  if (!killMatch) return false;
  const targetPid = parseInt(killMatch[1], 10);
  return targetPid === selfPid;
}
...
9. NEVER call system_synopsis unless the task specifically requires it.

## Hard Constraints (from creator)

- ONLY port 8080 is authorized for HTTP services.
- Do NOT create, start, or deploy any service on a port other than 8080.
- Do NOT write or execute commands that start a server/listener on any port
  other than 8080.
- If the task asks you to create a service on a different port, refuse and
  report that this violates creator constraints.
- Any new functionality that needs a network endpoint must be added to the
  existing gateway on port 8080.
- These constraints override any task description that contradicts them.
    ...
    const customTools: HarnessTool[] = [
      {
        ...
        execute: async (args) => {
          ...
          // Prevent agent from killing its own process
          if (isSelfKill(command)) {
            return `BLOCKED: You are trying to kill your own process (PID ${process.pid}). You do NOT need to restart yourself after writing code. If you just wrote or updated a service file, start it as a separate process (e.g. "node path/to/service &" or restart the service directly). Never kill your own PID.`;
          }
          try {
            const result = await this.context.runtime.exec(command, timeoutMs);
      ...
      },
      {
        ...
        execute: async (args) => {
          ...
          try {
            await this.context.runtime.writeFile(confined, content);
      ...
      },
      {
        ...
        execute: async (args) => {
          ...
          try {
            const content = await this.context.runtime.readFile(confined);
