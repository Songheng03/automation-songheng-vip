...
- Do NOT write code that starts a server/listener on any port other than 8080.
- If the task asks you to create a service on a different port, refuse and
  report that this violates creator constraints.
- Any new endpoint must be added to the existing gateway on port 8080.
      ...
      },
      {
        ...
        execute: async (args) => {
          ...
          try {
            let content: string;
            try {
              content = await this.context.runtime.readFile(confined);
            } catch {
      ...
      },
      {
        ...
        execute: async (args) => {
          ...
          try {
            ...
            try {
              await this.context.runtime.writeFile(confined, patched);
            } catch {
