...
function resolveAndValidatePath(filePath: string): string | null {
  try {
    ...
    // Step 3: Check resolved path is within an allowed base directory
    const baseDirs = [
      path.resolve(process.cwd()),
      path.resolve("/root/automaton"),
    ];
    const withinAllowedDir = baseDirs.some(
    ...
    if (fs.existsSync(resolved)) {
      ...
      const realWithinAllowed = baseDirs.some(
