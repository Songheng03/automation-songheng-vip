# Submission: my-automaton → Smithery

Submit at: https://smithery.ai/submit
Method: manual
Note: MCP server directory

---

# my-automaton

AI Code Review & Security Scanning — Pay-Per-Use From 1¢

## Description
my-automaton is a sovereign AI agent providing developer tools via REST API with x402 micropayments. Unlike SaaS subscriptions at $20-40/month, my-automaton charges 1¢-5¢/request with a free tier (3/day/IP).

Services: Code Review (5¢), Security Scan (3¢), Analysis (1¢), Summarization (2¢), Refactoring (5¢), Complexity Analysis (2¢), Batch Processing (5¢).

Integrations: REST API, CLI (npx @my-automaton/cli), GitHub Actions, MCP Server, OpenAI Tool Format.

Free tier: No signup, 3 free requests/day per IP.

Payments: USDC on Base chain via x402 protocol. Pay per request, no subscriptions.

## Links
- **Website**: https://automation.songheng.vip
- **Docs**: https://automation.songheng.vip/api-docs.html
- **GitHub**: https://github.com/my-automaton/cli
- **Logo**: https://automation.songheng.vip/favicon.ico

## Category
Developer Tools › Code Review

## Pricing
Free + Paid (from 1¢/request)

## Features
- Free tier: 3 code reviews/day, no signup
- Pay-per-use from 1¢ — no subscription
- x402 micropayments (USDC on Base chain)
- AI code review with quality scoring (0-100)
- Security vulnerability scanning (OWASP Top 10)
- Multi-language: JS, TS, Python, Solidity, Go, Rust, Java
- GitHub Actions integration for PR auto-review
- MCP server for Claude and AI agents
- CLI: npx @my-automaton/cli review --file app.js
- REST API with x402 USDC micropayments

## Tech Stack
DeepSeek AI, Node.js, Cloudflare Tunnel, x402 Protocol, Base chain, USDC

## Wallet
0x76eADdEBFfb6A61DD071f97F4508467fc55dd113 (Base · USDC)

